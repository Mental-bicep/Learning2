...
import com.profiles.exception.InsufficientBalanceException;
import com.profiles.kafka.KafkaProducerService;
import com.profiles.mapper.TicketEntityMapper;
import com.profiles.repository.BookingRepository;
import com.profiles.repository.PaymentRepository;
...
//		triggerAbruptCrash(); // comment out for happy flow
		...
		publishMessageToKafka(savedEntity.getPnr()); // Kafka Producer service call
		
		
		return savedEntity;
	}

	public void publishMessageToKafka(Integer pnr){
		for(int i=1; i<=200; ++i){ 
			String msg = "this is a test msg PNR generated is " + pnr;
			kafkaProducerService.publishMessage("booking-confirmed", msg);
		}
	}
